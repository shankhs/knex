...
function Formatter_Mongo() {
  this.client = client;
  Formatter.apply(this, arguments);
}
inherits(Formatter_Mongo, Formatter);

Formatter_Mongo.prototype.operators = [
  '=', '<', '>', '<=', '>=', '<>', '!=',
...
Formatter_Mongo.prototype.wrapValue = function(value) {
  return (value !== '*' ? '`' + value + '`' : '*');
};
...
Formatter_Mongo.prototype._wrap = wrapperMemo;

// Assign the formatter to the the client.
client.Formatter = Formatter_Mongo;
